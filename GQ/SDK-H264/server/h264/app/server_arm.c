/*
接口：V4L1
编码：T264    T264是国内的开源项目，T264 decoder的程序做过汇编优化，速度还可以，但只能解 T264 本身的码流。开
*/
...
void read_video(char *pixels,int w, int h)
{
 ...
 /* 对于单帧采集只需设置 frame=0*/
  cam_mm.frame = 0;
  cam_mm.format=VIDEO_PALETTE_YUV420P; //change by 180708
...
void init_param(T264_param_t* param, const char* file)
{
	...
	if (!(fd = fopen(file,"r")))//以只读方式打开文件，该文件必须存在。
	{
	...
	/*
	fgets函数：从文件结构体指针stream中读取数据，每次读取一行。
	sscanf函数：读取格式化的字符串中的数据。
	*/
